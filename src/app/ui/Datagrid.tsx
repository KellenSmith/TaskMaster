...
interface DatagridProps {
  ...
  deleteAction: (userId: string, currentActionState: DatagridActionState) => Promise<DatagridActionState>;
}

const Datagrid: React.FC<DatagridProps> = ({ name, fetchData, updateAction, deleteAction }) => {
  const apiRef = useGridApiRef()
  const pathname = usePathname()
  const [clickedRow, setClickedRow] = useState(null)
  const [fetchedDataState, setFetchedDataState] = useState<DatagridActionState>(defaultActionState);
  ...
  const updateDatagridData = async () => {
    ...
    if (newActionState.status !== 200) setDialogErrorMsg(newActionState.errorMsg)
  }
  ...
  const deleteRow = async ()=> {
    if (clickedRow[GlobalConstants.ID]===user[GlobalConstants.ID]) return setDialogErrorMsg("You can't delete your own user")
    const deleteState = await deleteAction(clickedRow[GlobalConstants.EMAIL], fetchedDataState)
    if (deleteState.status !== 200) return setDialogErrorMsg(deleteState.errorMsg)
    updateDatagridData()
  }
  ...
  return (
    <Stack sx={{height: '100%'}}>
      ...
      <Dialog open={!!clickedRow} onClose={closeDialog}>
        ...
        {
          !!dialogErrorMsg && <Typography color="error">{dialogErrorMsg}</Typography>
        }
        <Button color="error" onClick={deleteRow}>Delete</Button>
      </Dialog>
    </Stack>
  );
};
